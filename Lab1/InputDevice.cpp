...
InputDevice::InputDevice(GameFramework* inGame) : game(inGame)
{
	...
	Rid[0].hwndTarget = game->displayWin->hWnd;

	Rid[1].usUsagePage = 0x01;
	Rid[1].usUsage = 0x06;
	Rid[1].dwFlags = 0;   // adds HID keyboard and also ignores legacy keyboard messages
	Rid[1].hwndTarget = game->displayWin->hWnd;
...
void InputDevice::OnMouseMove(RawMouseEventArgs args)
{
	...
	POINT p;
	GetCursorPos(&p);
	ScreenToClient(game->displayWin->hWnd, &p);
